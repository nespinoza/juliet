...
class load(object):
    ...
    def __init__(self,priors = None, input_folder = None, t_lc = None, y_lc = None, yerr_lc = None, \
        ...
        if type(priors) == str:
            ...
            priors, n_transit, n_rv, numbering_transit, numbering_rv, n_params = readpriors(
                priors)
...
class fit(object):
    ...
    def __init__(self, data, use_ultranest = False, use_dynesty = False, dynamic = False, dynesty_bound = 'multi', dynesty_sample='rwalk', dynesty_nthreads = None, \
        ...
        if self.use_ultranest:
            ...
            paramnames = [
                pname for pname in self.model_parameters
                if self.data.priors[pname]['distribution'] != 'fixed'
            ]
            ...
            sampler = ReactiveNestedSampler(paramnames,
                                            ...
                                            log_dir=self.out_folder,
                                            resume=True)
            results = sampler.run(frac_remain=0.1,
                                  min_num_live_points=self.n_live_points,
            ...
            if runMultiNest:
                ...
                output = pymultinest.Analyzer(
                    outputfiles_basename=self.out_folder + 'jomnest_',
                    n_params=self.data.nparams)
            ...
            if runDynesty:
                if self.dynesty_nthreads is None:
                    ...
                    with contextlib.closing(Pool(processes=nthreads -
                                                 1)) as executor:
                        sampler = dynesty.DynamicNestedSampler(self.loglike, self.prior, self.data.nparams, \
        ...
        if runMultiNest or runDynesty or runUltraNest:
            ...
            if runDynesty:
                if self.dynamic and (self.out_folder is not None):
                    pickle.dump(
                        out,
                        open(self.out_folder + '_dynesty_DNS_posteriors.pkl',
                             'wb'))
                elif (not self.dynamic) and (self.out_folder is not None):
                    pickle.dump(
                        out,
                        open(self.out_folder + '_dynesty_NS_posteriors.pkl',
                             'wb'))
            else:
                if self.out_folder is not None:
                    pickle.dump(out,
                                open(self.out_folder + 'posteriors.pkl', 'wb'))
        else:
            ...
            if (self.use_dynesty) and (self.out_folder is not None):
                ...
                if self.data.pickle_encoding is None:
                    out = pickle.load(
                        open(self.out_folder + 'posteriors.pkl', 'rb'))
                else:
                    out = pickle.load(open(self.out_folder + 'posteriors.pkl',
                                           'rb'),
                                      encoding=self.data.pickle_encoding)
            if len(out.keys()) == 0:
                print(
...
class model(object):
                ...
                if t is None:
                    ...
                    if self.modeltype == 'lc':
                        ...
                        if self.global_model:
                            ...
                            for ginstrument in instruments:
                                if self.dictionary[ginstrument][
                                        'TransitFit'] or self.dictionary[
                                            ginstrument]['TransitFitCatwoman']:
                                    if not self.dictionary[ginstrument][
                                            'TransitFitCatwoman']:
                                        supersample_params[
                                            ginstrument], supersample_m[
                                                ginstrument] = init_batman(
                                                    t,
                                                    self.dictionary[ginstrument]
                                                    ['ldlaw'])
                                        sample_params[ginstrument], sample_m[
                                            ginstrument] = init_batman(
                                                self.times[ginstrument],
                                                self.dictionary[ginstrument]
                                                ['ldlaw'])
                            ...
                            if self.global_model:
                                ...
                                for ginstrument in instruments:
                                    if self.dictionary[ginstrument][
                                            'TransitFit'] or self.dictionary[
                                                ginstrument][
                                                    'TransitFitCatwoman']:
                                        self.model[ginstrument][
                                            'params'], self.model[ginstrument][
                                                'm'] = supersample_params[
                                                    ginstrument], supersample_m[
                                                        ginstrument]
                                ...
                                if self.dictionary[instrument][
                                    ...
                                    self.model[instrument][
                                        'params'], self.model[instrument][
                                            'm'] = supersample_params, supersample_m
                                if self.lm_boolean[instrument]:
                            ...
                            if not self.dictionary[instrument]['TTVs'][i][
                                ...
                                if self.log_like_calc:
                                    self.model[instrument]['M'] += self.model[
                                        instrument]['m'].light_curve(
                                            self.model[instrument]
                                            ['params']) - 1.
                                else:
                                    self.model[instrument][
                                        'p' + str(i)] = self.model[instrument][
                                            'm'].light_curve(
                                                self.model[instrument]
                                                ['params'])
                                    self.model[instrument]['M'] += self.model[
    ...
    def gaussian_log_likelihood(self, residuals, variances):
        taus = 1. / variances
        return -0.5 * (len(residuals) * log2pi + np.sum(-np.log(taus) + taus *
                                                        (residuals**2)))
