...
pout = os.getcwd() + '/juliet/juls/Lambert_2Inst_SamePC_TEST'
if SamePC_2Inst:
...
for ins in range(len(instruments)):
    ...
    sine_model = 1. + lambert_model * ((flx_ecl - 1.) / m2.fp)
...
if SamePC_2Inst:
    ...
    for ins in range(len(instruments)):
        par_pc = par_pc + ['aglambert_p1_' + instruments[ins]]
        dist_pc = dist_pc + ['uniform']
        hyper_pc = hyper_pc + [[0., 0.5]]
...
if SamePC_2Inst:
    data = np.vstack([ post1['p_p1'], post1['b_p1'], post1['q1_' + '_'.join(instruments)], post1['q2_' + '_'.join(instruments)],\
                    post1['a_p1'], post1['aglambert_p1'] ])
    data = np.transpose(data)
    lbls = np.array([ 'Rp/R*', 'b', 'q1', 'q2', 'a/R*', 'Ag'])
    truths = np.array([ rprs, bb, q1, q2, ar, Ags[0]])
else:
    ...
    data = np.transpose(data)
    lbls = np.array([ 'Rp/R*', 'b', 'q1', 'q2', 'a/R*', 'Ag0', 'Ag1' ])
    truths = np.array([ rprs, bb, q1, q2, ar, Ags[0], Ags[1] ])
