if __name__ == '__main__':
    ...
    # Kelp inhomogeneous reflection phase curve model simulation for 1/2 instrument
    ...
    pout = 'kelpinhomo_2Inst_diffPC'
    if SamePC_2Inst:
        ## Instrument 1 and 2 both have same phase curve parameters
        w0, wp = [0.3, 0.3], [0.6, 0.6]
        x1, x2 = [-19, -19], [46, 46]
        ag = [0.15, 0.15]
    else:
        ## Instrument 1 and 2 have different phase curve parameters
        w0, wp = [0.3, 0.4], [0.6, 0.5]
        x1, x2 = [-19, -29], [46, 38]
        ag = [0.15, 0.25]

    # If we want to use Morris (2024) parametrization for x1 and x2;
    # Then we would instead fit for x1prime and x2prime
    # Default is False, change it True if you want to use it
    morris24_parametrization = False
    ...
    for ins in range(len(instruments)):
        ...
        refl_fl_ppm, _, _ = reflected_phase_curve_inhomogeneous(phases=phases_sorted, omega_0=w0[ins], omega_prime=wp[ins],\
                                                                x1=np.radians(x1[ins]), x2=np.radians(x2[ins]), A_g=ag[ins], a_rp=ar/rprs)
        ...
        plt.plot(tim7, total_model, 'k-', zorder=10)
        plt.ylim([1-100e-6, 1+100e-6])
        plt.show()
        #plt.savefig(os.getcwd() + '/juliet/juls/kelpinhomo_1Inst.png')

        plt.plot(phases_sorted, total_model)
        plt.show()

        print(total_model)
        print(np.where(np.isnan(fle7)))
    ...
    if SamePC_2Inst:
        par_pc = ['w0_p1', 'wp_p1', 'agkelp_p1']
        dist_pc = ['uniform', 'uniform', 'uniform']
        hyper_pc = [[0., 1.], [0., 1.], [0., 0.6]]

        # Morris (2024) parametrization for x1 and x2
        if morris24_parametrization:
            par_pc = par_pc + ['x1prime_p1', 'x2prime_p1']
            dist_pc = dist_pc + ['uniform', 'uniform']
            hyper_pc = hyper_pc + [[-1., 0.], [0., 1.]]
        else:
            par_pc = par_pc + ['x1_p1', 'x2_p1']
            dist_pc = dist_pc + ['uniform', 'uniform']
            hyper_pc = hyper_pc + [[-90., 0.], [0., 90.]]
    else:
        par_pc, dist_pc, hyper_pc = [], [], []
        for ins in range(len(instruments)):
            par_pc = par_pc + ['w0_p1_' + instruments[ins], 'wp_p1_' + instruments[ins], 'agkelp_p1_' + instruments[ins]]
            dist_pc = dist_pc + ['uniform', 'uniform', 'uniform']
            hyper_pc = hyper_pc + [[0., 1.], [0., 1.], [0., 0.6]]
            
            # Morris (2024) parametrization for x1 and x2
            if morris24_parametrization:
                par_pc = par_pc + ['x1prime_p1_' + instruments[ins], 'x2prime_p1_' + instruments[ins]]
                dist_pc = dist_pc + ['uniform', 'uniform']
                hyper_pc = hyper_pc + [[-1., 0.], [0., 1.]]
            else:
                par_pc = par_pc + ['x1_p1_' + instruments[ins], 'x2_p1_' + instruments[ins]]
                dist_pc = dist_pc + ['uniform', 'uniform']
                hyper_pc = hyper_pc + [[-90., 0.], [0., 90.]]
    ...
    if SamePC_2Inst:
        if not morris24_parametrization:
            data = np.vstack([ post1['p_p1'], post1['b_p1'], post1['q1_' + '_'.join(instruments)], post1['q2_' + '_'.join(instruments)],\
                            post1['a_p1'], post1['w0_p1'], post1['wp_p1'], post1['x1_p1'], post1['x2_p1'], post1['agkelp_p1'] ])
        else:
            data = np.vstack([ post1['p_p1'], post1['b_p1'], post1['q1_' + '_'.join(instruments)], post1['q2_' + '_'.join(instruments)],\
                        post1['a_p1'], post1['w0_p1'], post1['wp_p1'], np.rad2deg(np.arcsin(post1['x1prime_p1'])),\
                        np.rad2deg(np.arcsin(post1['x2prime_p1'])), post1['agkelp_p1'] ])
        
        data = np.transpose(data)
        lbls = np.array([ 'Rp/R*', 'b', 'q1', 'q2', 'a/R*', 'w0', 'wp', 'x1', 'x2', 'ag'])
        truths = np.array([ rprs, bb, q1, q2, ar, w0[0], wp[0], x1[0], x2[0], ag[0] ])
    else:
        if not morris24_parametrization:
            data = np.vstack([ post1['p_p1'], post1['b_p1'], post1['q1_' + '_'.join(instruments)], post1['q2_' + '_'.join(instruments)],\
                            post1['a_p1'], post1['w0_p1_' + instruments[0]], post1['wp_p1_' + instruments[0]], post1['x1_p1_' + instruments[0]],\
                            post1['x2_p1_' + instruments[0]], post1['agkelp_p1_' + instruments[0]], post1['w0_p1_' + instruments[1]],\
                            post1['wp_p1_' + instruments[1]], post1['x1_p1_' + instruments[1]], post1['x2_p1_' + instruments[1]],\
                            post1['agkelp_p1_' + instruments[1]] ])
        else:
            data = np.vstack([ post1['p_p1'], post1['b_p1'], post1['q1_' + '_'.join(instruments)], post1['q2_' + '_'.join(instruments)],\
                            post1['a_p1'], post1['w0_p1_' + instruments[0]], post1['wp_p1_' + instruments[0]], np.rad2deg(np.arcsin(post1['x1prime_p1_' + instruments[0]])),\
                            np.rad2deg(np.arcsin(post1['x2prime_p1_' + instruments[0]])), post1['agkelp_p1_' + instruments[0]], post1['w0_p1_' + instruments[1]],\
                            post1['wp_p1_' + instruments[1]], np.rad2deg(np.arcsin(post1['x1prime_p1_' + instruments[1]])), np.rad2deg(np.arcsin(post1['x2prime_p1_' + instruments[1]])),\
                            post1['agkelp_p1_' + instruments[1]] ])
        
        data = np.transpose(data)
        lbls = np.array([ 'Rp/R*', 'b', 'q1', 'q2', 'a/R*', 'w0', 'wp0', 'x10', 'x20', 'ag0', 'w1', 'wp1', 'x11', 'x21', 'ag1'])
        truths = np.array([ rprs, bb, q1, q2, ar, w0[0], wp[0], x1[0], x2[0], ag[0], w0[1], wp[1], x1[1], x2[1], ag[1] ])
